...
app.MapGet("/resetpassword", async (HttpRequest request, HttpResponse response) => {
    ...
    var reply = await client.PostAsync($"https://localhost/webservice/reset/server.php?wstoken={wstoken}&wsfunction={wsfunction}&moodlewsrestformat={moodlewsrestformat}", stringContent);
